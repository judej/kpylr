...
class TestSyntaxFacts:
    def test_get_unary_operator_precedence(self) -> None:
        ...
        assert (
            SyntaxFacts.get_unary_operator_precedence(SyntaxKind.subtractiontoken) == 3
        )
        assert (
            SyntaxFacts.get_unary_operator_precedence(SyntaxKind.multiplicationtoken)
            == 0
        )
    ...
    def test_get_binary_operator_precedence(self) -> None:
        ...
        assert (
            SyntaxFacts.get_binary_operator_precedence(SyntaxKind.subtractiontoken) == 1
        )
        assert (
            SyntaxFacts.get_binary_operator_precedence(SyntaxKind.multiplicationtoken)
            == 2
        )
